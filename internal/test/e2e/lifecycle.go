...
// DefaultDeleteTimeout bounds how long cleanup waits for the controller to
// finalize a managed resource before logging the failure and moving on.
const DefaultDeleteTimeout = 2 * time.Minute

// Apply creates obj on the cluster, failing the test on any error. Callers
// that also want automatic cleanup + readiness wait should use
...
// Update sends a full update of obj, failing the test on any error. The
// object's resourceVersion must already be populated (call Get or re-fetch
// the object first).
...
// CreateAndWaitForReady creates obj, registers a t.Cleanup that deletes it and
// waits for the controller to drop the finalizer, then blocks until obj reports
// Ready+Synced.
//
// On a failed wait the test is marked Fatal but cleanup still runs so the next
...
// Failures are logged rather than failing the test — by the time cleanup runs
// the assertions of interest have already happened.
