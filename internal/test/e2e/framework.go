...
const (
	// EnvHarborURL is the base URL of the Harbor API used for verification.
	EnvHarborURL = "HARBOR_URL"
	// EnvHarborUsername is the Harbor admin username.
	EnvHarborUsername = "HARBOR_USERNAME"
	// EnvHarborPassword is the Harbor admin password.
	EnvHarborPassword = "HARBOR_PASSWORD"
	// EnvProviderConfig is the ClusterProviderConfig name that managed resources reference.
	EnvProviderConfig = "HARBOR_PROVIDERCONFIG"
)

// DefaultProviderConfigName is used when EnvProviderConfig is unset; it matches
// the ClusterProviderConfig provisioned by cluster/local/harbor_setup.sh.
const DefaultProviderConfigName = "e2e"

// Framework groups the clients every e2e test needs: a controller-runtime
// client wired to the project's schemes, and a Harbor API client authenticated
// with admin credentials. Construct one per test with New.
...
func New(t *testing.T) *Framework {
	...
	rawURL := mustEnv(t, EnvHarborURL)
	username := mustEnv(t, EnvHarborUsername)
	password := mustEnv(t, EnvHarborPassword)
